...
function FileTreeNode({ node, level }: FileTreeNodeProps) {
  ...
  return (
    <div>
      <div
        className={cn(
          "flex items-center gap-2 px-2 py-1.5 hover:bg-[#EDE9E3] cursor-pointer text-sm transition-colors",
          selectedFile === node.path && "bg-[#FBE9E0] text-[#D97757]"
        )}
      ...
      >
        {node.type === "directory" ? (
          <>
            ...
            {isExpanded ? (
              <FolderOpen className="h-4 w-4 shrink-0 text-[#D97757]" />
            ) : (
              <Folder className="h-4 w-4 shrink-0 text-[#D97757]" />
            )}
          </>
        ) : (
          <>
